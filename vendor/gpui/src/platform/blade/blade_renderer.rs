...
impl BladeRenderer {
    ...
    pub fn draw(&mut self, scene: &Scene) {
        ...
        if frame.image_index().is_none() {
            // The swapchain is out of date (e.g. the surface was configured by the
            // compositor after creation) and blade can no longer present it.
            // Recreate the swapchain and re-acquire instead of rendering into a
            // frame that will never be presented, which would freeze the window.
            self.out_of_date_recoveries = self.out_of_date_recoveries.saturating_add(1);
            if self.out_of_date_recoveries <= 3 {
                log::info!("swapchain out of date, reconfiguring surface");
                self.wait_for_gpu();
                self.gpu.reconfigure_surface(&mut self.surface, self.surface_config);
                frame = self.surface.acquire_frame();
            }
        } else {
            self.out_of_date_recoveries = 0;
        }
